...
function tampilkanRekomendasiAI() {
  ...
  if (jam >= 5 && jam < 11) {
    ...
    judul = "🌄 Menu Sarapan & Minuman Pagi";
  } else if (jam >= 11 && jam < 17) {
    kategoriRekomendasi = "makanan";
    judul = "☀️ Makan Siang Favorit";
...
if (order.test(text)) {
  ...
  let pesanan = [];
  try {
    const data = localStorage.getItem("pesananSaya");
    pesanan = data ? JSON.parse(data) : [];
    if (!Array.isArray(pesanan)) pesanan = []; // jaga-jaga kalau bukan array
  } catch (err) {
    console.warn("⚠️ Gagal membaca data pesanan:", err);
    pesanan = [];
  }
  ...
  if (pesanan.length === 0) {
    reply.text = `
      📦 Kamu belum punya pesanan aktif nih 😅<br>
  ...
  // Kalau ada pesanan → tampilkan daftar & total
  let total = 0;
  const daftar = pesanan
    .map(item => {
      const nama = item?.name || "Menu Tidak Dikenali";
      const harga = item?.price || 0;
      const qty = item?.qty || 1;
      total += harga * qty;
      return `🍴 ${nama} x${qty} (Rp ${(harga * qty).toLocaleString("id-ID")})`;
    })
    .join("<br>");

  reply.text = `
    ✅ Ini pesanan kamu saat ini:<br>${daftar}<br><br>
    💰 Total: <b>Rp ${total.toLocaleString("id-ID")}</b><br>
    Mau lanjut bayar, ubah, atau batalkan pesanan? 💳
  `;
  reply.quickReplies = ["Lanjut Bayar"];
  return reply;
}
